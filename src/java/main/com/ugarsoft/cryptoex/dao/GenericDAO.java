package com.ugarsoft.cryptoex.dao;
...
public abstract class  GenericDAO<E, K extends Serializable> { 
	...
	public  boolean create(  E t) {
		boolean msg = false;
		try {
			getCurrentSession().save(tclass.getName(), t);
	...
	public  boolean update( E  t) {
		boolean msg = false;
		try {
			getCurrentSession().update(tclass.getName(), t);
			msg = true;
		} catch (Exception e) {
	...
	public  boolean delete( E  t) {
		Boolean msg = false;
		try {
			getCurrentSession().delete(tclass.getName(), t);
			msg = true;
		} catch (Exception e) {
			e.printStackTrace();
		}
